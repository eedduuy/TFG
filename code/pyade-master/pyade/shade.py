...
def apply(population_size: int, individual_size: int, bounds: np.ndarray,
          ...
          max_evals: int, seed: Union[int, None], fitness: np.ndarray, init_pop: np.ndarray, prevm_cr=0, prevm_f=0, k=0) -> [np.ndarray, int]:
    """
    ...
    #func2= partial(func, model, dls)
    
    if np.isscalar(prevm_cr):
    ...
    archive = []
    k = k
    historic_best = [population[np.argmin(fitness)]]
    ...
    for current_generation in range(max_iters):
      ...
      p = np.random.uniform(low=2/(population_size*2), high=0.2, size=population_size)
      ...
      if gen%4==0:
        ...
        historic_fitness.append(fitness[min_ind])
        historic_best.append(population[min_ind])
    ...
    return population, fitness, m_cr, m_f, k, historic_best, historic_fitness
